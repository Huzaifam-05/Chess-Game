...
bool Pawn::valid(int s_row, int s_col, int d_row, int d_col, Board* board)
{
    ...
    if (d_col == s_col &&
        ((getColor() == WHITE && s_row == 6) || (getColor() == BLACK && s_row == 1)) &&
        d_row == s_row + 2 * direction)
    {
...
bool Rook::valid(int s_row, int s_col, int d_row, int d_col, Board* board)
{
    ...
    if (c_col == 0)
    {
        if (s_row > d_row) swap(s_row, d_row);
        for (int i = s_row + 1; i < d_row; i++)
            if (board->getPiece(i, s_col) != nullptr) return false;
    }
    if (c_row == 0)
    {
        if (s_col > d_col) swap(s_col, d_col);
        for (int i = s_col + 1; i < d_col; i++)
            if (board->getPiece(s_row, i) != nullptr) return false;
    }
    return true;
}
...
bool King::valid(int s_row, int s_col, int d_row, int d_col, Board* /*board*/)
{
    int c_row = abs(d_row - s_row);
    int c_col = abs(d_col - s_col);
    if (c_row == 0 && c_col == 0) return false;
...
void Board::setupPiece()
{
    ...
    grid[0][3] = new Queen(Color::BLACK);  grid[7][3] = new Queen(Color::WHITE);
    grid[0][4] = new King(Color::BLACK);   grid[7][4] = new King(Color::WHITE);
}

Piece* Board::getPiece(int r, int c) const { return grid[r][c]; }
void   Board::setPiece(int r, int c, Piece* p) { grid[r][c] = p; }
            ...
            if (grid[i][j] && grid[i][j]->getColor() == player)
                for (int r = 0; r < rows; r++)
                    for (int c = 0; c < cols; c++)
                        ...
                        {
                            if (grid[r][c] && grid[r][c]->getColor() == player) continue;
                            if (isMoveSafe(i, j, r, c, grid[i][j])) return true;
                        }
    return false;
}
...
void Board::display(int dangerRow, int dangerCol)
{
    ...
    for (int i = 0; i < rows; i++)
    {
        ...
        for (int j = 0; j < cols; j++)
        {
            ...
            if (i == dangerRow && j == dangerCol) cout << "\033[1;48;5;196m";
            else if (light)                            cout << "\033[1;48;5;247m";
            else                                       cout << "\033[1;48;5;239m";
